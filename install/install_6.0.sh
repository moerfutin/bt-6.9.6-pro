...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh $Umaru_Url/install/install-ubuntu_6.0.sh && bash install.sh
		exit;
	else
		wget -O install.sh $Umaru_Url/install/install-ubuntu_6.0.sh && sudo bash install.sh
		exit;
	fi
fi
